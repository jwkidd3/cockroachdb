#!/usr/bin/env bash
# Optional Kubernetes exercise with cockroach-operator on kind.
...
if ! command -v kind >/dev/null 2>&1 || ! command -v kubectl >/dev/null 2>&1; then
    ...
    echo "Optional K8s: ${PASS_COUNT} assertions passed, ${FAIL_COUNT} failed (live portion skipped)."
...
section "Done"
echo "Optional K8s: ${PASS_COUNT} assertions passed, ${FAIL_COUNT} failed."
[ "$FAIL_COUNT" -eq 0 ]
